...
function StormCheckContent() {
  ...
  const checkStormData = useCallback(async (addressToCheck: string, monthsToCheck: number = 6) => {
    ...
    try {
      const response = await fetch('/api/storm-check', {
        ...
        body: JSON.stringify({ address: addressToCheck, months: monthsToCheck }),
      })
  ...
  useEffect(() => {
    ...
    if (urlAddress && !hasAutoChecked) {
      ...
      checkStormData(urlAddress, months)
    }
