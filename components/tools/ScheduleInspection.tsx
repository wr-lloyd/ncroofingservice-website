...
export default function ScheduleInspection({ onSubmit, urgency = 'normal', prefilledIssue }: ScheduleInspectionProps) {
  ...
  if (isSubmitted) {
    return (
      <div className="bg-slate-900/50 rounded-2xl border border-green-500/30 p-8 text-center">
        ...
        <h3 className="text-2xl font-bold text-white mb-2">Inspection Requested!</h3>
        ...
        <div className="bg-slate-800/50 rounded-xl p-4 mb-6">
          ...
          <ol className="text-slate-400 text-sm text-left space-y-2">
            <li className="flex items-start gap-2">
              <span className="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 text-xs text-white">1</span>
              We&apos;ll call to confirm your preferred date and time
            </li>
            <li className="flex items-start gap-2">
              <span className="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 text-xs text-white">2</span>
              A certified inspector will visit your property
            </li>
            <li className="flex items-start gap-2">
              <span className="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 text-xs text-white">3</span>
              You&apos;ll receive a detailed assessment and options
            </li>
          </ol>
        </div>
        <p className="text-slate-500 text-sm">
          Can&apos;t wait? Call us directly: <a href="tel:+19194758841" className="text-blue-400 hover:underline">(919) 475-8841</a>
        </p>
      </div>
    )
  }
  ...
  return (
    <div className={`bg-slate-900/50 rounded-2xl border ${urgency === 'priority' ? 'border-orange-500/30' : 'border-white/5'} p-8`}>
      {urgency === 'priority' && (
      ...
      <h3 className="text-2xl font-bold text-white mb-2">Schedule Your Free Inspection</h3>
      <p className="text-slate-400 mb-6">Tell us when works best and we&apos;ll confirm your appointment.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="preferredDate" className="block text-sm font-medium text-slate-300 mb-1">Preferred Date *</label>
            <input
              type="date"
              id="preferredDate"
              name="preferredDate"
          ...
          <div>
            <label htmlFor="preferredTime" className="block text-sm font-medium text-slate-300 mb-1">Preferred Time *</label>
            <select
              id="preferredTime"
              name="preferredTime"
