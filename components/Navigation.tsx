        ...
        {isOpen && (
          <div className="lg:hidden pb-4">
            {navLinks.map((link) => (
              <Link
                ...
                className={`block py-3 transition-colors font-medium border-b border-white/5 ${
                  link.highlight ? 'text-orange-400' : 'text-slate-300 hover:text-white'
                }`}
            ...
            <div className="mt-4 space-y-3">
              ...
              <Link
                href="/contact"
                className="flex items-center justify-center gap-2 border border-white/20 hover:bg-white/10 text-white px-6 py-3 rounded-lg font-semibold transition-colors"
                onClick={() => setIsOpen(false)}
              >
                Free Inspection
              </Link>
